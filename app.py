...
# Random defaults
# ----------------------------------
DEFAULT_GATES = ["C31", "C24", "D1", "D3", "B5", "B10", "A11", "A9", "E2", "E4"]
...
def chat():
    ...
    if request.method == "POST":
        ...
        session["flight_number"]  = (request.form.get("flight_number")  or "").strip() or "Unknown"
    ...
    if session.pop("just_scanned", False):
        ...
        destination     = "London (LHR)"  # TODO: map from flight if available
        time_to_flight  = session.get("time_to_flight") or random_hours_4_to_12()
        gate            = session.get("gate") or random_gate()

        first_msg = (
            f"Hi {passenger_name}, your next flight to {destination} departs in {time_to_flight} "
